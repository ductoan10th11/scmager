...
const statusClass = (status) => ({
  ...
  PENDING_RECONCILIATION: 'border-violet-200 bg-violet-50 text-violet-700',
  PENDING_SCORE: 'border-amber-200 bg-amber-50 text-amber-700',
  UNRESOLVED_PERFORMER: 'border-zinc-300 bg-zinc-100 text-zinc-700',
...
const metricCards = computed(() => [
  ...
  { label: 'Đã xử lý', value: summary.value.completedDocuments, note: 'Đã hoàn thành và chốt trạng thái', icon: CheckCircle2, tone: 'bg-emerald-50 text-emerald-700' },
  { label: 'Đang xử lý', value: summary.value.inProgressDocuments, note: 'Gồm các mục chờ đối soát', icon: Clock3, tone: 'bg-amber-50 text-amber-700' },
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 px-4 py-3"><div><h2 class="text-sm font-bold text-zinc-900">Nhiệm vụ trong kỳ</h2><p class="mt-0.5 text-xs text-zinc-500">Nhiệm vụ và công việc ngoài hệ thống được tính theo hạn xử lý.</p><p v-if="summary.unmappedDocuments" class="mt-1 text-xs font-semibold text-amber-700">{{ formatNumber(summary.unmappedDocuments) }} nhiệm vụ chưa map được sang tài khoản nội bộ, vẫn hiển thị đầy đủ bên dưới.</p></div><TabsList aria-label="Lọc nhiệm vụ trong kỳ"><TabsTrigger value="all" class="text-xs font-bold">Tất cả</TabsTrigger><TabsTrigger value="active" class="text-xs font-bold">Đang xử lý</TabsTrigger><TabsTrigger value="completed" class="text-xs font-bold">Đã xử lý</TabsTrigger></TabsList></div>
            <TabsContent v-for="tab in documentTabs" :key="tab.value" :value="tab.value" class="m-0 overflow-x-auto">
              <Table class="min-w-[1180px]">
                <TableHeader class="bg-zinc-50 text-xs font-semibold text-zinc-500"><TableRow><TableHead>Văn bản</TableHead><TableHead>Người xử lý</TableHead><TableHead>Trạng thái</TableHead><TableHead class="text-right">Điểm</TableHead><TableHead>Hạn xử lý</TableHead><TableHead>Nộp kết quả</TableHead></TableRow></TableHeader>
                <TableBody class="divide-y divide-zinc-100">
                  <TableRow v-for="document in tab.documents" :key="document.id" tabindex="0" role="button" class="cursor-pointer hover:bg-zinc-50/70 focus:bg-blue-50 focus:outline-none" @click="openDocument(document)" @keydown.enter="openDocument(document)" @keydown.space.prevent="openDocument(document)">
                    <TableCell class="max-w-[430px]"><div class="flex items-center gap-2"><span v-if="document.soDen" class="shrink-0 rounded-full bg-zinc-900 px-1.5 py-0.5 text-[11px] font-bold text-white">SĐ {{ document.soDen }}</span><span class="truncate font-bold text-zinc-900">{{ document.soKyHieu || document.documentId }}</span></div><p class="mt-1 line-clamp-2 text-xs leading-5 text-zinc-600">{{ document.trichYeu || 'Không có trích yếu.' }}</p></TableCell>
                    <TableCell><p class="font-semibold text-zinc-800">{{ document.owner?.fullName || 'Chưa map được người xử lý' }}</p><p class="mt-0.5 text-xs text-zinc-500">{{ document.owner?.position || document.owner?.username || '—' }}</p></TableCell>
                    <TableCell><span class="inline-flex whitespace-nowrap rounded-full border px-2 py-1 text-xs font-bold" :class="statusClass(document.status)">{{ statusLabel(document.status) }}</span></TableCell>
                    <TableCell class="text-right"><p class="font-bold text-blue-700">{{ formatNumber(document.kpiEligible === false ? document.point : document.completed ? document.creditedPoint : document.point) }}</p><p v-if="document.kpiEligible === false" class="mt-0.5 text-xs text-amber-700">chưa chốt KPI</p><p v-else-if="document.completed && document.point !== document.creditedPoint" class="mt-0.5 text-xs text-rose-600">gốc {{ formatNumber(document.point) }}</p></TableCell>
                    <TableCell><p class="font-semibold text-zinc-800">{{ formatDate(document.deadline) }}</p><p v-if="document.lateWorkingDays" class="mt-0.5 text-xs font-medium text-rose-600">Trễ {{ document.lateWorkingDays }} ngày làm việc</p></TableCell>
                    <TableCell class="text-xs text-zinc-600"><p>{{ formatDateTime(document.submittedAt) }}</p><p v-if="document.completedAt" class="mt-1 text-zinc-400">Hoàn tất: {{ formatDateTime(document.completedAt) }}</p></TableCell>
                  </TableRow>
                  <TableRow v-if="!tab.documents.length"><TableCell colspan="6" class="py-12 text-center text-zinc-400">Không có văn bản phù hợp trong kỳ này.</TableCell></TableRow>
                </TableBody>
              </Table>
            </TabsContent>
          </Tabs>
        ...
        <div v-if="selectedDocument || selectedAssignee" class="fixed inset-0 z-[80]">
          ...
          <aside class="absolute right-0 top-0 flex h-full w-full max-w-[580px] flex-col border-l border-zinc-200 bg-white shadow-2xl shadow-zinc-950/20">
            ...
            <div v-else-if="selectedDocument" class="min-h-0 flex-1 overflow-auto p-5">
              ...
              <section class="border-b border-zinc-100 py-4"><h3 class="text-xs font-bold uppercase text-zinc-400">KPI và kết quả</h3><dl class="mt-3 space-y-2 text-sm"><div class="flex justify-between gap-4"><dt class="text-zinc-500">Người nhận điểm</dt><dd class="text-right font-semibold text-zinc-800">{{ selectedDocument.owner?.fullName || selectedDocument.processing?.currentAssignee?.fullName || selectedDocument.processing?.currentAssignee?.externalFullName || 'Chưa map được' }}</dd></div><div v-if="selectedDocument.reconciliation" class="flex justify-between gap-4"><dt class="text-zinc-500">Đối soát sản phẩm</dt><dd class="text-right font-semibold text-zinc-800">{{ statusLabel(selectedDocument.status) }}</dd></div><div v-if="selectedDocument.reconciliation?.lastError" class="flex justify-between gap-4"><dt class="text-zinc-500">Lý do chờ</dt><dd class="max-w-[320px] text-right text-amber-700">{{ selectedDocument.reconciliation.lastError }}</dd></div><div class="flex justify-between gap-4"><dt class="text-zinc-500">Nộp kết quả</dt><dd class="text-right font-semibold text-zinc-800">{{ formatDateTime(selectedDocument.submittedAt) }}</dd></div><div class="flex justify-between gap-4"><dt class="text-zinc-500">Hoàn tất</dt><dd class="text-right font-semibold text-zinc-800">{{ formatDateTime(selectedDocument.completedAt) }}</dd></div></dl></section>
