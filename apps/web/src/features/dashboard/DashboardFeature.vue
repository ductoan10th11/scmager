...
const taskSummary = ref({ total: 0, todo: 0, inProgress: 0, pendingReview: 0, done: 0, overdue: 0 })
const completionRate = computed(() => (
  taskSummary.value.total
    ? Math.round((taskSummary.value.done / taskSummary.value.total) * 100)
    : 0
))
const kpis = computed(() => [
  { title: 'Nhiệm vụ', value: taskSummary.value.total, icon: CheckSquare, color: 'text-blue-600', bg: 'bg-blue-50' },
  { title: 'Chờ xử lý', value: taskSummary.value.todo, icon: Calendar, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { title: 'Đang làm', value: taskSummary.value.inProgress, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
  { title: 'Hoàn thành', value: `${completionRate.value}%`, icon: Activity, color: 'text-indigo-600', bg: 'bg-indigo-50' }
])
const workSuggestions = computed(() => {
  const items = []
  if (taskSummary.value.overdue > 0) {
    items.push({
      color: 'bg-rose-500',
      title: `${taskSummary.value.overdue} nhiệm vụ quá hạn`,
      detail: 'Ưu tiên kiểm tra và xử lý các nhiệm vụ đã vượt deadline.',
    })
  }
  if (taskSummary.value.todo > 0) {
    items.push({
      color: 'bg-blue-500',
      title: `${taskSummary.value.todo} nhiệm vụ đang chờ`,
      detail: 'Mở danh sách nhiệm vụ để bắt đầu công việc được giao.',
    })
  }
  if (taskSummary.value.inProgress > 0) {
    items.push({
      color: 'bg-amber-500',
      title: `${taskSummary.value.inProgress} nhiệm vụ đang thực hiện`,
      detail: 'Hoàn thiện kết quả và gửi trưởng phòng duyệt khi sẵn sàng.',
    })
  }
  if (items.length === 0) {
    items.push({
      color: 'bg-emerald-500',
      title: 'Không có nhiệm vụ tồn đọng',
      detail: 'Các nhiệm vụ hiện tại đã được xử lý.',
    })
  }
  return items
})
...
const chatMessages = ref([
  { id: 1, role: 'ai', text: 'Chào bạn! Tôi có thể giúp tổng hợp nhiệm vụ và lịch làm việc hôm nay.' }
])
...
const currentUserName = computed(() => user.value?.fullName || user.value?.email || 'Người dùng')
  ...
  <div class="h-full flex flex-col bg-zinc-50/50 overflow-hidden">
    ...
    <div class="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto w-full flex flex-col gap-6 lg:gap-8 h-full overflow-hidden">
      ...
      <div class="shrink-0 mt-2">
        <h2 class="text-2xl md:text-3xl font-bold text-zinc-900 tracking-tight text-left">Chào buổi sáng, {{ currentUserName }}.</h2>
            ...
            >
              ...
              <div class="flex-1 w-full overflow-hidden relative min-h-0 mt-3">
                    ...
                    <div class="flex-1 flex flex-col gap-4 overflow-y-auto hide-scrollbar min-h-0 pt-1">
                      ...
                      <div class="flex flex-col gap-2.5 shrink-0 pb-2">
                        <div
                          ...
                          class="flex items-start gap-3 bg-white/70 p-3 md:p-3.5 rounded-2xl border border-white shadow-sm"
                        >
                          <div :class="['w-2 h-2 rounded-full mt-1.5 shrink-0', suggestion.color]"></div>
                          <div class="flex-1 min-w-0">
                            <p class="text-[13px] md:text-sm font-bold text-zinc-900 truncate mb-0.5">{{ suggestion.title }}</p>
                            <p class="text-xs md:text-[13px] text-zinc-600 font-medium leading-relaxed line-clamp-2">{{ suggestion.detail }}</p>
                          </div>
                        </div>
                      </div>
                </div>
                ...
                <div class="mt-2 shrink-0 relative bg-white p-[5px] rounded-full shadow-sm border border-indigo-50 flex items-center gap-1 transition-all focus-within:shadow-md focus-within:border-indigo-200">
                  ...
                  <Input 
                    v-model="aiChatInput"
                    placeholder="Hỏi về nhiệm vụ, lịch làm việc..." 
